...
book_page_urls.each do |url|
  client.query({"input"=>{"webpage/url"=>url},"connectorGuids"=>["cc356d73-acd2-4ecd-86ec-f2d37adfb93e"]}, callback )
  #sleep 1
end
...
CSV.open("data/es_meta_results.csv", "w") do |csv|
  # Write header
  csv << ["title", "authors", "book_description", "meta_all"]
  ...
  (i...json_page_length).each do 
    (n...json_file[i].length).each do 
      csv << json_file[i][n].values_at("title", "authors", "book_description", "meta_all")
      n += 1
    end
    n = 0
    i += 1
