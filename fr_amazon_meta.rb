...
def connect_and_get_response
  # Connect and receive and parse response. 
  # In the case of Timeout Error, retry 3 times.
  Retriable.retriable on: [Timeout::Error, RuntimeError], tries: 4 do
    Net::HTTP.start(@uri.host, @uri.port, :use_ssl => true) do |http|
      ...
      if @parsed_response["results"].empty?
        puts "Empty!"
        raise 'Returned with empty results!'
      elsif @parsed_response["results"].nil?
        raise 'Nil results!'
      end
...
system('say -v Alex "Work is done come back to work again" ')