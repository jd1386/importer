...
book_page_urls.each do |url|
  client.query({"input"=>{"webpage/url"=>url},"connectorGuids"=>["7922a573-a329-4026-981d-c554dd1615f0"]}, callback )
end
...
puts "Done! The results saved to data/it_meta_results.json."

# Check if every book from every page has been scraped
json_file = JSON.parse(File.open("data/it_meta_results.json").read)
...
CSV.open("data/it_meta_results.csv", "w") do |csv|
  # Write header
  csv << ["_pageUrl", "title", "authors_publisher", "cover_image", "cover_image/_source", "cover_image/_title", "cover_image/_alt", "book_description", "meta_1", "meta_2"]
  ...
  (i...json_page_length).each do 
    (n...json_file[i].length).each do 
      csv << json_file[i][n].values_at("_pageUrl", "title", "authors_publisher", "cover_image", "cover_image/_source", "cover_image/_title", "cover_image/_alt", "book_description", "meta_1", "meta_2")
      n += 1
    end
    n = 0
    i += 1
...
puts "The results saved to data/it_meta_results.csv"
