...
book_page_urls = []
File.readlines('data/no_meta_source.txt').each do |line|
  book_page_urls << line
end
...
book_page_urls.each do |url|
  client.query({"input"=>{"webpage/url"=>url},"connectorGuids"=>["d5234923-81d9-4ac7-893e-7dc7fa35023d"]}, callback )
end
...
File.new('data/no_meta_results.json', 'w') unless File.exists?('data/no_meta_results.json')
File.open('data/no_meta_results.json', 'w') do |f|
  f << JSON.pretty_generate(data_rows)
end
puts "Done! The results saved to data/no_meta_results.json."

# Check if every book from every page has been scraped
json_file = JSON.parse(File.open("data/no_meta_results.json").read)
...
CSV.open("data/no_meta_results.csv", "w") do |csv|
  # Write header
  csv << ["title", "sub_title", "authors", "title_and_authors_all", "book_description", "meta_all"]
  ...
  (i...json_page_length).each do 
    (n...json_file[i].length).each do 
      csv << json_file[i][n].values_at("title", "sub_title", "authors", "title_and_authors_all", "book_description", "meta_all")
      n += 1
    end
    n = 0
    i += 1
...
puts "The results saved to data/no_meta_results.csv"
