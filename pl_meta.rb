...
book_page_urls = []
File.readlines('data/pl_meta_source.txt').each do |line|
  book_page_urls << line
end
...
book_page_urls.each do |url|
  client.query({"input"=>{"webpage/url"=>url},"connectorGuids"=>["be92ce9a-ce39-4b0c-b56c-77d01f65e08b"]}, callback )
end
...
File.new('data/pl_meta_results.json', 'w') unless File.exists?('data/pl_meta_results.json')
File.open('data/pl_meta_results.json', 'w') do |f|
  f << JSON.pretty_generate(data_rows)
end
puts "Done! The results saved to data/pl_meta_results.json."

# Check if every book from every page has been scraped
json_file = JSON.parse(File.open("data/pl_meta_results.json").read)
...
CSV.open("data/pl_meta_results.csv", "w") do |csv|
  # Write header
  csv << ["title", "book_description", "category", "meta_1", "meta_2"]
  ...
  (i...json_page_length).each do 
    (n...json_file[i].length).each do 
      csv << json_file[i][n].values_at("title", "book_description", "category", "meta_1", "meta_2")
      n += 1
    end
    n = 0
    i += 1
...
puts "The results saved to data/pl_meta_results.csv"
