...
File.new('data/pl_recent_results.json', 'w') unless File.exists?('data/pl_recent_results.json')
File.open('data/pl_recent_results.json', 'w') do |f|
  f << JSON.pretty_generate(data_rows)
end

puts "Done! The results saved to data/pl_recent_results.json."

json_file = JSON.parse(File.open("data/pl_recent_results.json").read)
...
CSV.open("data/pl_recent_results.csv", "w") do |csv|
  # Write header
...
puts "The results saved to data/pl_recent_results.csv"
