...
File.readlines('/Users/jungdolee/projects/importer/data/to_hash_source.txt', encoding: 'UTF-8').each do |line|
 ## Pre-process
  ...
  new_line = new_line.gsub("Editor ", " || Publisher=> ") 
  new_line = new_line.gsub("Data de lançamento ", " || Pub_date=> ") 
  new_line = new_line.gsub("Nº Coleção ", " || Edition=> ") 
  new_line = new_line.gsub("Coleção ", " || Collection=> ") 
  new_line = new_line.gsub("ISBN ", " || ISBN=> ") 
  new_line = new_line.gsub("EAN ", " || EAN=> ") 
  new_line = new_line.gsub("Marca ", " || Brand=> ") 
  new_line = new_line.gsub("Dimensões ", " || Dimension=> ") 
  new_line = new_line.gsub("Nº Páginas ", " || Pages=> ") 
  new_line = new_line.gsub("Encadernação ", " || Binding=> ") 
  new_line = new_line.gsub("Peso ", " || Weight=> ") 
  new_line = new_line.gsub("Idade Recomendada ", " || Age_group=> ") 
  new_line = new_line.gsub("Conteúdo Inclui ", " || Including=> ") 
...
CSV.open("/Users/jungdolee/projects/importer/data/to_hash_results.csv", "w") do |csv|
	# Write header
  csv << [ "Author", "Illustrator", "Translator", "Publisher", "Pub_date", "Collection", "ISBN", "Edition", "Dimension", "Pages", "Binding", "Weight", "Age_group", "Including" ]

  # Write rows
  i = 0

  (0...meta_all.length).each do 
  	csv << meta_all[i][0].values_at( "Author", "Illustrator", "Translator", "Publisher", "Pub_date", "Collection", "ISBN", "Edition", "Dimension", "Pages", "Binding", "Weight", "Age_group", "Including" )
  	i += 1
  end
