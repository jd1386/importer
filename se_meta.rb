...
book_page_urls = []
File.readlines('data/se_meta_source.txt').each do |line|
  book_page_urls << line
end
...
book_page_urls.each do |url|
  client.query({"input"=>{"webpage/url"=>url},"connectorGuids"=>["03281a4a-2a3b-453c-8d18-203251952b0a"]}, callback )
end
...
File.new('data/se_meta_results.json', 'w') unless File.exists?('data/se_meta_results.json')
File.open('data/se_meta_results.json', 'w') do |f|
  f << JSON.pretty_generate(data_rows)
end
puts "Done! The results saved to data/se_meta_results.json."

# Check if every book from every page has been scraped
json_file = JSON.parse(File.open("data/se_meta_results.json").read)
...
CSV.open("data/se_meta_results.csv", "w") do |csv|
  # Write header
...
puts "The results saved to data/se_meta_results.csv"
