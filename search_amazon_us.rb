...
File.new('data/search_amazon_us_results.json', 'a') unless File.exists?('data/search_amazon_us_results.json')

# Open the file and append the data results to results_file.json
File.open('data/search_amazon_us_results.json', 'a') do |f|
  f << JSON.pretty_generate(data_rows)
end

# Now we have the results file in json format.
puts "The data is appended to the search_amazon_us_results.json file."